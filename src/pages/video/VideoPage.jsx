...
export default function VideoPage() {
  ...
  // keyboard transport: space play/pause · ↑/↓ jump to in/out · i/o set in/out
  useEffect(() => {
    const onKey = (e) => {
      const v = videoRef.current
      if (!v) return
  ...
  return (
    <div className="min-h-dvh bg-surface-primary text-emphasis flex">
      ...
      >
        ...
        {srcUrl && (
          <>
            ...
            {meta && (
              <div className="max-w-[920px] w-full mx-auto flex flex-col gap-2">
                <div className="flex items-center gap-3">
                  ...
                  <span className="kol-mono-10 text-meta tabular-nums">
                    {meta.width}×{meta.height}{out ? ` → ${out.w}×${out.h}` : ''} · clip {clipLen.toFixed(2)}s · space · ↑↓ ends
                  </span>
                </div>
                <Timeline
      ...
      {/* ── rail ── */}
      <EditorRail>
        <RailHeader>{file ? file.name : 'Video'}</RailHeader>
        ...
        <Section label="Project">
          <Dropdown size="sm" variant="subtle" className="w-full" value={projectKey} onChange={changeProject}
          ...
          {kind === 'free' && <p className="kol-mono-10 text-meta">drag the box on the video</p>}
        </Section>
        ...
        <Section label="Quality">
          ...
          <p className="kol-mono-10 text-meta">
            {kind !== 'source'
              ? `${kind === 'free' ? 'crop' : 'format'} re-encodes (precise)`
              : mode === 'fast'
                ? 'stream-copy · instant, lossless, cuts on keyframes'
        ...
        <p className="kol-mono-10 text-body">trim · reframe to format · drop a clip, set in/out, export</p>
      </EditorRail>
    </div>
  )
}
