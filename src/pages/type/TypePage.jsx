...
// Type — a blank typesetting canvas: start empty, drop text, style it with the full
// Kinetic toolkit (font · axes · morph · pattern · italic) and NONE of the preset /
// gallery machinery. Reuses Kinetic's engine + the Design/Layout/Edit control trio
// verbatim; this is only the blank-start shell.
...
export default function TypePage() {
  ...
  const [instances, setInstances] = useState(() => [newInstance('t1', theme0.fg)])
  const [selId, setSelId] = useState('t1')
  const [frameBg, setFrameBg] = useState(theme0.bg)
  ...
  const addInstance = (type = 'line') => {
    ...
    setInstances((arr) => [...arr, defaultInstance(id, { text: 'Text', font: 'rot', fontSize: 140, fill: arr[0]?.fill || theme0.fg, path: { type } })])
    setSelId(id)
  }
  ...
  useEffect(() => { if (!instances.find((x) => x.id === selId)) setSelId(instances[0]?.id) }, [instances, selId])
    ...
    <div className="flex min-h-dvh">
      <div className="flex-1 min-w-0 h-dvh bg-surface-secondary overflow-hidden">
        ...
        >
          ...
          {selected && !editing && <InstancePositioner offset={selected.offset} stage={stage} onChange={(o) => setInst(selId, 'offset', o)} />}
          {selected && !editing && (
            <SelectionFrame
              engineRef={engineRef} selId={selId} stage={stage} instance={selected}
              ...
              onSize={(v) => setInst(selId, 'fontSize', v)}
      ...
      <LiveClock getT={() => progressRef.current.t}>
        <EditorRail header={header} footerBare footer={footer}>
          ...
          {tab === 'motion' && (
            selected
              ? <MotionControls params={selected} setMotion={(k, v) => setInstMotion(selId, k, v)} />
