...
export default function DiscoPage() {
  return (
    <SynthShell engineClass={DiscoEngine} title="Disco" name="disco" defaults={DEFAULTS}>
      {(p, update) => (
        <Section label="Disco">
          <Slider labeled label="Kaleidoscope" min={0} max={12} step={1} value={p.segments} onChange={(v) => update('segments', roundIfNum(v))} variant="default" />
          <Slider labeled label="Hue cycle" min={0} max={2} step={0.05} value={p.hue} onChange={(v) => update('hue', v)} variant="default" />
          <Slider labeled label="Spin" min={-1} max={1} step={0.02} value={p.spin} onChange={(v) => update('spin', v)} variant="default" />
