...
export default function FxParamControl({ name, spec, value, onChange }) {
  if (spec.options) {
    return (
  ...
  return (
    <Slider labeled label={name} min={spec.min} max={spec.max} step={spec.step} value={value} onChange={onChange} variant="default" raised />
  )
}
